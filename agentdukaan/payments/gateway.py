...
class MockGateway:
    ...
    def create_payment(self, order: dict) -> GatewayResult:
        ...
        audit.log(
            ...
            decision="ok",
            detail={"mode": "mock", "note": "instant capture simulation"},
        )
        return GatewayResult(
            provider=self.provider, ref=ref, status="paid", checkout_url=None
        )
        ...
        rp_order = self.client.order.create(
            {
                ...
                "notes": {
                    "mission_id": order.get("mission_id") or "",
                    "source": "agentdukaan-mcp",
                },
            }
        )
