...
<a href="/">Storefront</a><a href="/dashboard">Dashboard</a>
...
def storefront():
    ...
    cards = []
    for p in products:
        mrp = f'<span class="mrp">{rupees(int(p["unit_price_rupees"]*1.2))}</span>' if False else ""
        cards.append(f"""
    ...
    tools = [
        ("get_store_manifest()", "Discover the store: categories, trust limits, guidance"),
        ...
        ("quote_order(items, pincode)", "Exact GST-inclusive quote — computed in code, never by the model"),
        ("create_order(quote_id, idempotency_key)", "Price-drift + stock checks; idempotent by UNIQUE key"),
        ("request_payment(order_id)", "Policy engine → PENDING human approval. Agent cannot pay."),
        ("get_order_status(order_id)", "State + full audited event timeline"),
        ("open_mission(brief, budget_rupees)", "Declare intent + hard budget the trust plane enforces"),
    ]
    tool_cards = "".join(
...
def dashboard():
    s = commerce.stats()
    with db.conn() as c:
        orders = c.execute("SELECT * FROM orders ORDER BY created_at DESC LIMIT 20").fetchall()
        pending = c.execute(
    ...
    if not approvals_html:
        approvals_html = '<p class="muted">No pending approvals. The gate is closed and waiting.</p>'
...
@app.get("/health")
def health():
    return {"ok": True, "service": "agentdukaan", "gateway": "razorpay-test" if settings.gateway_is_live else "mock"}


@app.post("/api/approvals/{approval_id}")
def decide_approval(approval_id: str, approved: str = "true", approver: str = "dashboard-human"):
    result = commerce.decide_approval(
        approval_id=approval_id, approved=approved.lower() in ("true", "1", "yes"), approver=approver
    )
...
async def razorpay_webhook(request: Request):
    ...
    if not hmac.compare_digest(expected, got):
        audit.log(actor="system", plane="trust", action="webhook.signature_check",
                  payload={}, decision="blocked", detail={"reason": "BAD_SIGNATURE"})
    ...
    if event.get("event") == "payment_link.paid":
        ...
        with db.conn() as c:
            order = c.execute("SELECT * FROM orders WHERE order_id = ?", (ref_id,)).fetchone()
        if order and order["status"] == "pending_payment":
            ...
            with db.conn() as c:
                c.execute(
                    ...
                    " WHERE order_id=?", (payment_id, now, ref_id),
                )
                if order["mission_id"]:
                    c.execute("UPDATE missions SET spent_paise = spent_paise + ? WHERE mission_id=?",
                              (order["total_paise"], order["mission_id"]))
                c.execute("UPDATE buyers SET spent_today_paise = spent_today_paise + ?"
                          " WHERE buyer_id='buyer_demo'", (order["total_paise"],))
            audit.log(actor="system", plane="trust", action="webhook.payment_captured",
                      payload={"order_id": ref_id, "payment_id": payment_id},
                      decision="ok", detail={"provider": "razorpay-test-mode"})
