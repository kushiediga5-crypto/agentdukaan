...
def test_price_drift_blocked():
    ...
    with db.conn() as c2:
        c2.execute(
            "UPDATE products SET unit_price_paise = 1 WHERE product_id = 'prd_eaa_watermelon'"
        )
    ...
    with db.conn() as c2:  # restore
        c2.execute(
            "UPDATE products SET unit_price_paise = 109_900 WHERE product_id = 'prd_eaa_watermelon'"
        )
...
def test_mission_budget_blocked():
    ...
    assert mission["ok"]
    quote = c.quote_order(
        [{"product_id": "prd_rope", "qty": 1}], "600001"
    )  # ₹299 + ship
    order = c.create_order(quote_id=quote["quote_id"], idempotency_key="budget-1")
    pay = c.request_payment(
        order_id=order["order"]["order_id"], mission_id=mission["mission_id"]
    )
    assert not pay["ok"]
...
def test_rejection_recovers_gracefully():
    """Human rejects => order marked 'rejected' (truthful for the agent) => the
    agent may re-request, and the retry needs a fresh human approval => paid."""
    c = Commerce()
    ...
    reject = c.decide_approval(
        approval_id=pay["approval_id"], approved=False, approver="test-human"
    )
    assert reject["ok"] and reject["status"] == "rejected"
    assert c.get_order(oid)["order"]["status"] == "rejected"

    pay2 = c.request_payment(order_id=oid)
    approve = c.decide_approval(
        approval_id=pay2["approval_id"], approved=True, approver="test-human"
    )
